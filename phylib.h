...
//Represents vector in 2 dimensions 
typedef struct {
    double x;
    double y;
} phylib_coord;

//Represents still ball. It has a number and position
typedef struct {
    unsigned char number;
    phylib_coord pos;
} phylib_still_ball;

//Represents rolling ball. It has a number, position, velocity and acceleration due to friction.
typedef struct {
    unsigned char number;
    phylib_coord pos;
    phylib_coord vel;
...
//Represents the coordinates of the holes 
typedef struct {
    phylib_coord pos;
} phylib_hole;
...
//Union can store any type of object
typedef union {
...
//To identify class of the object 
typedef struct {
    phylib_obj type;
    phylib_untyped obj;
} phylib_object;
...
//Functions to allocate memory for the different objects.
...
//Functions for stimulating the balls movement 
